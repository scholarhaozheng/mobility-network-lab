...
<div class="soft"><div class="wrap"><section class="evidence-support"><div class="sectionhead"><h2>Supporting mobility data</h2><p>Open transit, map-feature and shared-mobility evidence helps select and document sources for a city study. Explore the retained research results, then use the local tools with your own records.</p></div><!-- EVIDENCE_OVERVIEW --><p class="note">Source-qualified research snapshots, not live coverage. These layers use different units and must not be added together. The public package contains summaries and selected code, not all source records or feeds. <a href="omdv-provenance.html">Trace the metrics →</a></p><div class="actions"><a class="button" href="open-data.html">Explore open mobility evidence →</a><a class="button" href="city-workflow.html">Place evidence in a city model →</a></div><div class="split"><div><p class="eyebrow">Executable data tools</p><h3>Normalize local city and catalog records</h3><p>Transparent named-entity matching preserves unmatched and ambiguous records. The result is metadata—not GPS map matching or an OD estimate.</p><a href="data-tools.html">Data tools →</a></div><div><pre><code>python -m pip install -r requirements-data-tools.txt
